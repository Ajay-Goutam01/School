...
import { Trophy, Plus, Trash2, ArrowUp, ArrowDown } from 'lucide-react';

const AdminActivities = () => {
  const [activities, setActivities] = useState([]);
  const [form, setForm] = useState({ title: '', category: 'Sports', description: '', image: '', imageId: '', isVisible: true });

  const fetchAct = async () => {
    try {
      const res = await api.get('/activities/all');
      if (res.data && res.data.success) setActivities(res.data.data);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ title: '', category: 'Sports', description: '', image: '', imageId: '', isVisible: true });
      fetchAct();
  ...
  const handleToggleVisibility = async (item) => {
    try {
      await api.put(`/activities/${item._id}`, { isVisible: !item.isVisible });
      fetchAct();
  ...
  const handleMoveOrder = async (index, direction) => {
    ...
    try {
      await Promise.all([
        api.put(`/activities/${current._id}`, { order: targetIdx }),
        api.put(`/activities/${target._id}`, { order: index })
      ]);
      fetchAct();
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold font-serif text-slate-900">Activities & Sports Management</h1>
        <p className="text-xs text-slate-500">Manage co-curricular activities, athletics, and cultural events.</p>
      </div>

      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl shadow-subtle border border-slate-200 space-y-4 max-w-2xl">
        <h3 className="text-base font-bold font-serif text-slate-900">Add New Activity</h3>
        ...
        {/* IMAGE UPLOADER */}
        <ImageUploader
          label="Activity Photo (ImageKit Upload)"
          value={form.image}
          fileId={form.imageId}
        ...
        {activities.map((a, idx) => (
          <div key={a._id} className="p-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              {a.image && <img src={a.image} alt={a.title} className="w-12 h-12 object-cover rounded-xl shrink-0" />}
              <div>
                <span className="text-[10px] font-bold text-gold-600 uppercase">{a.category}</span>
                <h4 className="text-sm font-bold font-serif text-slate-900">{a.title}</h4>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className={`px-3 py-1 rounded text-xs font-semibold ${a.isVisible ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'}`}
              >
              ...
              <button type="button" onClick={() => handleDelete(a._id)} className="text-rose-600 p-1"><Trash2 className="w-4 h-4" /></button>
            </div>
          </div>
        ))}
      </div>
